...
- `cache warm` hashes local text-like cache segments, classifies stable versus dynamic context, includes deterministic Voxel TriWiki context in the stable prefix when `voxel index` has written `.opensks/triwiki/voxels.jsonl`, and writes `cache-hit-report.json` plus `cache-layout-improvement.json` by comparing the current stable prefix with the previous warm snapshot. `prod-001` passes only when `cache-hit-report.json` proves local stable-prefix reuse met the >=95% target with provider metrics still explicitly not connected. The Voxel TriWiki cache-layout gate requires the `.opensks/triwiki/voxels.jsonl` segment; stable-prefix reuse alone is not enough for that scoped gate.
